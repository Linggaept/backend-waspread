...
export class CreateBlastReplyTable1769950000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_blast_replies_blastId_createdAt" ON "blast_replies" ("blastId", "createdAt")`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_blast_replies_blastMessageId" ON "blast_replies" ("blastMessageId")`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_blast_replies_phoneNumber_receivedAt" ON "blast_replies" ("phoneNumber", "receivedAt")`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "blast_replies" DROP CONSTRAINT IF EXISTS "FK_blast_replies_blastMessageId"`,
    );
    await queryRunner.query(
      `ALTER TABLE "blast_replies" DROP CONSTRAINT IF EXISTS "FK_blast_replies_blastId"`,
    );

    // Drop indexes (if exists)
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_blast_replies_phoneNumber_receivedAt"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_blast_replies_blastMessageId"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "IDX_blast_replies_blastId_createdAt"`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "blasts" DROP COLUMN IF EXISTS "replyCount"`,
    );
  }
}
